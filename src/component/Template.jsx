...
//                     <span className=" text-orange-500 rounded-full h-6 text- font-medium px-2 py-1 uppercase">
//                         TEMPLATE
//                     </span>
//                 </div>
...
function Template() {
    return (
        <section id='template'>
            <div className="mt-30 p-4 ">
                {/* Badge Title */}
                <div className="text-center mb-8  lg:mt-20 tracking-wide">
                    <span className="text-orange-500 rounded-full h-6 text-xl font-medium px-2 py-1 uppercase">
                        LIST TEMPLATE
                    </span>
                </div>

                {/* Sidebar List - Sekarang di atas */}
                <div className="w-full p-4 rounded-md shadow-md text-center mb-5">
                    {/* <h3 className="text-lg font-semibold mb-4 text-white-800">List Template</h3> */}
                    <ul className="flex flex-wrap justify-center gap-4 text-white-700">
                        <li className="hover:text-orange-500 cursor-pointer">Digital</li>
                        <li className="hover:text-orange-500 cursor-pointer">Fashion</li>
                        <li className="hover:text-orange-500 cursor-pointer">Agribisnis</li>
                        <li className="hover:text-orange-500 cursor-pointer">Aksesoris</li>
                ...
                <div className="w-full p-4 rounded-md shadow-md">
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        {pictures.map((picture, index) => (
                            <div key={index} className="w-64 h-64 overflow-hidden group relative shadow-md hover:shadow-2xl hover:-translate-y-1 hover:shadow-[0_0_30px_rgba(255,255,200,0.7)] shadow-orange-400 mx-auto">
                                <div>
                                    <img
                                        src={picture.img}
                                        alt="Scroll image"
                                        className="transform group-hover:translate-y-[-50%] transition-transform duration-[3000ms] ease-in-out"
                                    />
                                </div>
                                <a href="#" className="hover cursor-pointer absolute bottom-0 left-0 right-0 bg-white text-black text-center text-blue-600 font-semibold py-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    USE TEMPLATE
                                </a>
                            </div>
                        ))}
